...
const registerUser:RequestHandler = async(req, res) => {
    try {
        ...
        if (!fullName || !userName || !email || !password){
            res.status(400).json({sucess: false, message: "Please provide all the required fields"})
            return
        }

        const [existingUser] = await db.query(`SELECT email FROM users WHERE email=?`, [email]) as any[]
        ...
        await db.query(`insert into users (full_name, user_name, email, password) values(?,?,?,?)`, [fullName, userName, email, hashedPassword])
...
const loginUser:RequestHandler = async(req, res) => {
    try {
        ...
        const [user] = await db.query(`select * from users where email=(?)`, [email]) as any[]
        ...
        res.status(200).json({success: true, message:"Login Successfully", data: otherData})
        return
